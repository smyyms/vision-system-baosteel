...
def cross_ratio_filter(inner_circle_diameter, outer_circle_diameter, tolerance=0.1):
    ...
    """
    annulus_cr = _cross_ratio(-0.5 * outer_circle_diameter, -0.5 * inner_circle_diameter, 0.5 * inner_circle_diameter,
                              0.5 * outer_circle_diameter)
...
class AnnulusDetection(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.minimum_inner_circle_size = kwargs.pop("minimum_inner_circle_size",
                                                    8)  # Minimum size in pixel of inner circle
        self.minimum_outer_circle_size = kwargs.pop("minimum_outer_circle_size",
                                                    16)  # Minimum size in pixel of outer circle
        self.relative_outer_inner_size = kwargs.pop("relative_outer_inner_size",
                                                    4)  # Maximum difference in size between outer and inner circle
        self.border_distance = kwargs.pop("border_distance", 5)  # Minimum distance in pixel to image border

        self.minimum_circle_points = kwargs.pop("minimum_circle_points",
                                                20)  # Minimum number of points for fitting circle
    ...
    def _find_candidates(self, annulus_areas, background_areas, image_shape):
        ...
        def get_background(annulus):
            bg_cand = None
            for background in background_areas[1:, (
            cv2.CC_STAT_LEFT, cv2.CC_STAT_TOP, cv2.CC_STAT_WIDTH, cv2.CC_STAT_HEIGHT, cv2.CC_STAT_AREA)]:
                ...
                # Cotained in ellipse
                if background[0] < annulus[0] or background[1] < annulus[1] or background[0] + background[2] > annulus[
                    0] + annulus[2] or background[1] + background[3] > annulus[1] + annulus[3]:
                    continue

                if annulus[2] > self.relative_outer_inner_size * background[2] or annulus[
                    3] > self.relative_outer_inner_size * background[3]:
                    return None
        ...
        candidates = []
        for annulus in annulus_areas[1:,
                       (cv2.CC_STAT_LEFT, cv2.CC_STAT_TOP, cv2.CC_STAT_WIDTH, cv2.CC_STAT_HEIGHT, cv2.CC_STAT_AREA)]:
            ...
            if annulus[0] <= self.border_distance or annulus[0] + annulus[2] >= image_shape[1] - self.border_distance or \
                    annulus[1] <= self.border_distance or annulus[1] + annulus[3] >= image_shape[
                0] - self.border_distance:
                continue
    ...
    def _fit_annuli(self, image, annuli_list, rect_list):
        ...
        def find_hq_circle(circle, rect, inner):
            ...
            for i in range(len(points)):
                if pt[i, 0] < rect[0] - 1 or pt[i, 0] > rect[0] + rect[2] + 1 or pt[i, 1] < rect[1] - 1 or pt[i, 1] > \
                        rect[1] + rect[3] + 1:
                    continue
    ...
    def _sample_ellipse(self, ellipse, rect):
        ...
        return np.row_stack(
            (np.column_stack((x, y1)), np.column_stack((x, y2)), np.column_stack((x1, y)), np.column_stack((x1, y))))
...
class Grid(object):
    ...
    def _get_homography_candidates(self, ellipse):
        ...
        # cr_grid = 1 - (d/m)**2
        cr_grid = _cross_ratio(0, self.outer_circle_diameter, self.marker_spacing + self.outer_circle_diameter,
                               self.marker_spacing)
    ...
    def _get_homography_from_candidate(self, ellipse, candidate):
        ...
        cr_grid = _cross_ratio(0, self.outer_circle_diameter, 2 * self.marker_spacing + self.outer_circle_diameter,
                               2 * self.marker_spacing)
        ...
        for i in range(3):
            ...
            if np.isclose(cr, cr_grid, rtol=0.05):
                ...
                src = np.row_stack(
                    [ellipse[center][0], ellipse[ax1_1][0], ellipse[ax1_2][0], ellipse[ax2_1][0], ellipse[ax2_2][0]])
...
def find_numbering(binary_image, H, grid, pattern=None):
    ...
    * = filled
    ...
    coord = np.array(
        [[-0.5, -0.5], [0, -0.5], [0.5, -0.5], [-0.5, 0], [0, 0], [0.5, 0], [-0.5, 0.5], [0, 0.5], [0.5, 0.5]])
