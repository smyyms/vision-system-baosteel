...
from UserInterface.TemperatureMeasure import Ui_TemperatureMeasure
from UserInterface.TemperatureMeasureInitialMain import TemperatureMeasureInitialServer


#Image Process
from bgimage.KR_C_Utils import *
...
class TemperatureMeasureServer(QMainWindow, Ui_TemperatureMeasure):
    def __init__(self, parent=None):
        super(TemperatureMeasureServer, self).__init__(parent)
        self.setupUi(self)
        self.init_Ui = TemperatureMeasureInitialServer()
        #self.showMaximized()
        ...
        image_ori = cv2.imread(KR_C_source_file + "/" + KR_C_demo_filename)
        ...
        #pixmap = QPixmap(KR_C_source_file + "/" + KR_C_demo_filename).scaled(self.ImageShow.width(),self.ImageShow.height())
        ...
        while True:
            ...
            if len(data) > 0:
                ...
                if recv_data_pack.status == 0:
                    ...
                    result = KR_C_process(image_ori)
                    image = QPixmap("./../sources/KR_C_display.png").scaled(self.ImageShow.width(),
                                                                            self.ImageShow.height())
                    self.signal_show_image.emit(image)
                    send_data_pack.sendstatus = result[0]
                    send_data_pack.z = result[1]
                    # status = 0
                    # TODO status
                s1 = struct.Struct('B')
                s2 = struct.Struct('B')
                s3 = struct.Struct('>h')
                send_buff = ctypes.create_string_buffer(4 * s1.size + s2.size + s3.size)
                ...
                s2.pack_into(send_buff, 4 * s1.size, send_data_pack.sendstatus)
                s2.pack_into(send_buff, 4 * s1.size + s2.size, send_data_pack.z)

                msg = str(send_data_pack.heartbeat) + " " + str(send_data_pack.camera_id) + " " + str(send_data_pack.program_id) + " " + \
                      str(send_data_pack.status) + " " + str(send_data_pack.sendstatus) + " " + str(send_data_pack.z)
...
if __name__ == "__main__":
    ...
    #初始化
    myWin = TemperatureMeasureServer()
    #将窗口控件显示在屏幕上
    myWin.show()
